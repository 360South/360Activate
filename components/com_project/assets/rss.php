...
class rss_feed {
  ...
  public function create_feed() {
    ...
    $now =  date("Y-m-d"); // get current time  // configure appropriately to your environment
  ...
  public function get_feed_items($rss_date,$rss_items_count = 20) {
    ...
    $a_topic_ids = array();
    $sql = 'SELECT id FROM cm0x8_project_items ' .
      'WHERE date <= ' . "'" . $conn->real_escape_string($rss_date) . "'" .
    ...
    foreach($a_topic_ids as $topic_id) {
      ...
      $sql='SELECT * FROM cm0x8_project_items WHERE id=' . $topic_id;
      $rs=$conn->query($sql);
      ...
      // link
      $a_rss_item['link'] = 'http://www.bluefish444.com/media-centre/latest-project.html';
      ...
      if($topic['image']) {
        ...
        $a_rss_item['description'] = '<img src="http://www.bluefish444.com/images/thumbnail/' . $img_url . '" hspace="5" vspace="5" align="left" />';
      }
      $a_rss_item['description'] .= htmlspecialchars(strip_tags($topic['body']));

      // pubdate -> configure appropriately to your environment
      $date = new DateTime($topic["date"]);
      $a_rss_item['pubDate'] = $date->format("D, d M Y");
