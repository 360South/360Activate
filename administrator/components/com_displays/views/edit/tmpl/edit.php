...
#echo '<pre>';print_r($this->item->displays);echo '</pre>';
?>
...
js(document).ready(function(){
  ...
  js('input:hidden.displays').each(function(){
	var name = js(this).attr('name');
	if(name.indexOf('displayshidden')){
	  if (js(this).val() > 0) {
	    js('#jform_displays option[value="'+js(this).val()+'"]').attr('selected',true);
	  }
	}
  });
  js('#jform_displays').chosen().change(function(){
	if(js('#jform_displays option:selected').length == 0){
	  js("#jform_displays option[value='']").attr('selected','selected');
	}
	if (js(this).val() == null) {
	  js('input.displays').remove();
	  js('#jform_displays option').each(function(){
	    js(this).attr('selected',false);
	  });
	  js('#jform_displays option').first().attr('selected','selected');
	}
  });
  js("#jform_displays").trigger("liszt:updated");
...
<form action="<?php echo JRoute::_('index.php?option=com_displays&layout=edit&id=' . (int) $this->item->id); ?>" method="post" enctype="multipart/form-data" name="adminForm" id="edit-form" class="form-validate">
