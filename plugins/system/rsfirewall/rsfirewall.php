...
class plgSystemRsfirewall extends JPlugin
{
	...
	public function onBeforeApiRoute()
	{
		...
		if (!$this->isWhitelisted())
		{
			...
			if ($this->isApiInjection())
			{
				...
				$this->reason = JText::_('COM_RSFIREWALL_UNAUTHORIZED_API_MESSAGE');
				$this->showForbiddenMessage();
			}
		}
	}
	...
	public function onAfterRender()
    {
        ...
        if (!$this->config || $app->isClient('administrator') || !$this->config->get('active_scanner_status') || !$this->config->get('verify_emails'))
        {
            return;
        }
