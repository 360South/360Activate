...
.install-not-ok {
	...
	<div>
		...
		<p><img src="<?php echo JUri::root(true) . '/media/com_rsfirewall/images/rsfirewall-box.png'; ?>" alt="RSFirewall!" /></p>
			...
			if (isset($messages['plg_console']))
			{
				...
				<p>System - RSFirewall! CLI Plugin ...
					<?php if ($messages['plg_console']) { ?>
						<b class="install-ok">Installed</b> <a href="index.php?option=com_plugins&task=plugin.edit&extension_id=<?php echo $messages['plg_console']; ?>" class="btn btn-secondary">Edit Plugin</a> Please enable manually if you wish to use the CLI.
					<?php } else { ?>
			...
			<h2>Changelog v3.0.12</h2>
			<ul class="version-history">
                <li><span class="version-new">New</span> Can now perform scans through the RSFirewall! CLI Plugin (Joomla! 4 only).</li>
                <li><span class="version-fixed">Fix</span> In some cases the 'Protect the following users from any changes' feature could throw an SQL error.</li>
			</ul>
			<?php
			if ($type === 'install')
			{
