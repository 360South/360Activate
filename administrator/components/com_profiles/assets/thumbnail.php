...
$ex     				= explode('.', $name);
$extension				= end($ex);
$sha                    = substr(sha1(time()),0,6);
$filename				= substr($_POST['name'],0,-(strlen($extension) + 1)).'-'.$sha.'.'.$extension;
$filenameMobile			= substr($_POST['name'],0,-(strlen($extension) + 1)).'-'.$sha.'-mobile.'.$extension;
