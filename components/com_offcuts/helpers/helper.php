...
abstract class OffcutsHelper {
	...
	public static function getTitle($id) {
		...
		if ($id) :
			...
			$query->from($db->quoteName('#__offcuts_items', 's'));
	...
	public static function getPrevNext()
	{
		...
		$query->select('*');
		$query->from('#__menu');
		$query->where('link = "index.php?option=com_offcuts&view=details"');
		$query->where('published = 1');
		...
		if ($count > 1) :
			...
			$links[0]->title = OffcutsHelper::getTitle( get_prev( $id, $all) );
			$links[0]->link  = JRoute::_( 'index.php?&Itemid=' . get_prev( $id, $all) );
			$links[0]->intro = OffcutsHelper::getIntro( get_prev( $id, $all) );
			$links[0]->image = OffcutsHelper::getImage( get_prev( $id, $all) );
			
			$links[1]->title = OffcutsHelper::getTitle( get_next( $id, $all) );
			$links[1]->link  = JRoute::_( 'index.php?&Itemid=' . get_next( $id, $all) );
			$links[1]->intro = OffcutsHelper::getIntro( get_next( $id, $all) );
			$links[1]->image = OffcutsHelper::getImage( get_next( $id, $all) );
